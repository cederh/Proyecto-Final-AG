...
//Variables de estado para mostrar los planetas.
int countSistema=0,
    countSol=0,
    countMercurio=0,
    countVenus=0,
...
void display(){
    ...
    if(countSistema == 0){
    	sistemaSolar();
    			...
    			solInfo();
    		}else if(countMercurio==1){
    				mercurioInfo();
    			}else if(countVenus==1){
    					venusInfo();
    				}else if(countTierra==1){
    						tierraInfo();
    					}else if(countLuna==1){
    							lunaInfo();
    						}else if(countMarte==1){
    								marteInfo();
    							}else if(countJupiter==1){
    									jupiterInfo();
    								}else if(countSaturno==1){
    										saturnoInfo();
    									}else if(countUrano==1){
    											uranoInfo();
    										}else if(countNeptuno==1){
    												neptunoInfo();
    											}
...
void sistemaSolar(){
    ...
    glRotated(rotSistema, 1.0, 0.0, 0.0);
    ...
    //Ayuda
    if(timeA <=25 ){
    sprintf(text, "PRESIONE CLICK DERECHO PARA EL MENU AYUDA");
    glColor3f(1, 1, 1);
    glRasterPos3f( -80 ,250 , 0);
    for(int i = 0; text[i] != '\0'; i++)
        glutBitmapCharacter(GLUT_BITMAP_HELVETICA_18, text[i]);
    }
    //ejes();
...
}else if(showPlanets == 1){

    glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);

    sprintf(text, "Sol");
    glColor3f(1, 1, 1);
    glRasterPos3f( 0 , 270, 0);
    for(int i = 0; text[i] != '\0'; i++)
        glutBitmapCharacter(GLUT_BITMAP_HELVETICA_18, text[i]);
...
void ayuda(){
    ...
    sprintf(text, "Bienvenido al Menu ayuda:");
    for(int i = 0; text[i] != '\0'; i++)
        glutBitmapCharacter(GLUT_BITMAP_HELVETICA_18, text[i]);
    ...
    sprintf(text, "Tecla E: Adelanta el tiempo."); 
    for(int i = 0; text[i] != '\0'; i++)
    	glutBitmapCharacter(GLUT_BITMAP_HELVETICA_18, text[i]); 

    glRasterPos3f( -280 , 50 , 0);
    sprintf(text, "Tecla R: Atrasa el tiempo."); 
    for(int i = 0; text[i] != '\0'; i++)
    	glutBitmapCharacter(GLUT_BITMAP_HELVETICA_18, text[i]); 

    glRasterPos3f( -280 , 30 , 0);
    sprintf(text, "Tecla T: Amplia las orbitas."); 
    for(int i = 0; text[i] != '\0'; i++)
    	glutBitmapCharacter(GLUT_BITMAP_HELVETICA_18, text[i]); 

    glRasterPos3f( -280 , 10 , 0);
    sprintf(text, "Tecla Y: Reduce las orbitas."); 
    for(int i = 0; text[i] != '\0'; i++)
    	glutBitmapCharacter(GLUT_BITMAP_HELVETICA_18, text[i]); 

    glRasterPos3f( -280 , -10 , 0);
    sprintf(text, "Tecla U: Oculta las orbitas."); 
    for(int i = 0; text[i] != '\0'; i++)
    	glutBitmapCharacter(GLUT_BITMAP_HELVETICA_18, text[i]);

    glRasterPos3f( -280 , -30 , 0);
    sprintf(text, "Tecla I: Muestra las orbitas."); 
    for(int i = 0; text[i] != '\0'; i++)
    	glutBitmapCharacter(GLUT_BITMAP_HELVETICA_18, text[i]); 
    ...
    sprintf(text, "Para mirar la informacion de los  planetas presione las siguientes teclas"); 
    for(int i = 0; text[i] != '\0'; i++)
    	glutBitmapCharacter(GLUT_BITMAP_HELVETICA_18, text[i]); 

    glRasterPos3f( -280 , -110 , 0);
    sprintf(text, "0 - Sol.     1 - Mercurio.     2 - Venus.     3 - Tierra.     4 - Marte."); 
    for(int i = 0; text[i] != '\0'; i++)
    	glutBitmapCharacter(GLUT_BITMAP_HELVETICA_18, text[i]); 


    glRasterPos3f( -280 , -130 , 0);
    sprintf(text, "5 - Jupiter.     6 - Saturno.     7 - Urano.     8 - Neptuno.     9 - Luna"); 
    for(int i = 0; text[i] != '\0'; i++)
    	glutBitmapCharacter(GLUT_BITMAP_HELVETICA_18, text[i]); 

    glRasterPos3f( -280 , -160 , 0);
    sprintf(text, "Para ver con mas detalles los planeta presione +. Para salir de la vista presione -."); 
    for(int i = 0; text[i] != '\0'; i++)
    	glutBitmapCharacter(GLUT_BITMAP_HELVETICA_18, text[i]);

   glRasterPos3f( -280 , -180 , 0);
    sprintf(text, "Puede desplazarce libremente con las teclasde direccion y alejar y acercar el planeta."); 
    for(int i = 0; text[i] != '\0'; i++)
      glutBitmapCharacter(GLUT_BITMAP_HELVETICA_18, text[i]);

    glRasterPos3f( -280 , -210 , 0);
    sprintf(text, "Para salir de la informacion presione la tecla Espacio"); 
    for(int i = 0; text[i] != '\0'; i++)
    	glutBitmapCharacter(GLUT_BITMAP_HELVETICA_18, text[i]);

    glRasterPos3f( -280 , -230 , 0);
    sprintf(text, "Para salir del Menu ayuda presione cualquier tecla"); 
    for(int i = 0; text[i] != '\0'; i++)
        glutBitmapCharacter(GLUT_BITMAP_HELVETICA_18, text[i]);
...
void initTex(){
    ...
    texTierraNoche = loadTex("Texturas/TierraNoche.jpg");
}
